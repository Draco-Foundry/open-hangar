...
~~JSON export/import~~ → ~~initial logo~~ → store catalog & prices (incl. warbonds)
→ finer item-type classification → broader browser support (Firefox polish +
Safari) → an opt-in API for approved sites to read the database.
